...
bool res:: modifier_res(int id,QString Type, QString date, QString horaire, int id_ligne)
{
    QString id1 = QString :: number (id) ;
      QString idl = QString :: number (id_ligne) ;
        ...
        QSqlQuery q;
         q.prepare("UPDATE RESERVATIONS set ID = :id , TYPE = :type , DATE_REV= :date ,HORAIRE= :horaire ,ID_LIGNE= :id_ligne where ID = :id");
         q.bindValue(":id", id);
...
model->setQuery("SELECT * from RESERVATIONS ORDER BY ID ASC");
model->setHeaderData(0, Qt::Horizontal, QObject::tr("ID"));
model->setHeaderData(1, Qt::Horizontal, QObject::tr("TYPE "));
model->setHeaderData(2, Qt::Horizontal, QObject::tr("DATE_REV"));
model->setHeaderData(3, Qt::Horizontal, QObject::tr("HORAIRE"));
...
QSqlQueryModel * res::trier_res_2()
{QSqlQueryModel * model= new QSqlQueryModel();

model->setQuery("SELECT * from RESERVATIONS ORDER BY TYPE ASC");
model->setHeaderData(0, Qt::Horizontal, QObject::tr("ID"));
model->setHeaderData(1, Qt::Horizontal, QObject::tr("TYPE "));
model->setHeaderData(2, Qt::Horizontal, QObject::tr("DATE_REV"));
model->setHeaderData(3, Qt::Horizontal, QObject::tr("HORAIRE"));
